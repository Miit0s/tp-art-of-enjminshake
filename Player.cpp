...
void Player::update(double deltaTime) {
	...
	if (haveToShoot) {
		...
		sf::RectangleShape rect(sf::Vector2f(distance * C::GRID_SIZE, laserHeight * C::GRID_SIZE));

		float yOffset = (C::GRID_SIZE - (C::GRID_SIZE * laserHeight)) / 2.0f;

		if (currentDirection == right)
			rect.setPosition((startPosition.x + 1) * C::GRID_SIZE, startPosition.y * C::GRID_SIZE + yOffset);
		else 
			rect.setPosition((hitPosition.x + 1) * C::GRID_SIZE, startPosition.y * C::GRID_SIZE + yOffset);

		rect.setFillColor(sf::Color{ 255, 20, 20, 255 });
		lasersSprite.push_back(rect);
